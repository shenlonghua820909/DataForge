...
└── docs/
    ...
    ├── PRODUCTION-READINESS-ASSESSMENT.md ← 🏭 清洗链路商用生产级就绪度评估（真/假闭环逐项对照 + 8 类差距清单 + 分阶段路径）
    └── INTEGRATION-MODULE-REVIEW.md ← 🧩 数据集成模块功能闭环与对标核查（断点定位 + 大厂功能对照 + 补齐建议）
```
...
- 🎛 **清洗页是「真操作」而非演示**：步骤可拖拽排序（移动前做可达性校验，不合法就拦下并说明原因）、全量变更走统一通道因此**任意操作都能撤销/重做**（含一键套用规则模板）、`⌘Z / ⌘⇧Z / ⌘S / ⌘/ / ⇧⌘K` 等快捷键（按 `?` 看全量清单）、日期歧义格式弹出人工确认而非静默猜测；**多表批处理**按「语义角色」而非表名做同类判定，一套模板可套到多张源表并逐表真实产出指标与 SQL，翻译不了的规则显式标为「不适用 / 需人工确认」，绝不静默丢弃
...
- 🧭 **静默失效零容忍**：668 条运行时 DOM 断言覆盖全部四档改造 + 数据集成模块 + 多轮 UX 简化（含「批处理结果与执行顺序无关」「批处理结束后现场原样还原」「停用正在回放的步骤必给解释性 toast」「toast hover 暂停」「默认日期口径不视为待确认」「❓ helper 默认折叠所有 cfg-note」「左栏单表 UI + 面包屑表名即切源表触发器」「顶栏不再有第二个『切换源表』按钮」「切表后面包屑表名同步刷新」「源表切换浮层的搜索/键盘/点击提交/后果预知纯读不污染配置」「切换器候选换成集成注册表（29 张 / 12 个数据源）后，分组、徽标、计数全部按结构自适应」「标着『未配清洗任务』的表点了/回车都不切表、只给引导 toast」「`?table=` 直达：已配任务就切过去、没配就停在默认表」「跨域扩到 10 张源表后所有断言仍按结构自适应」「拖拽上传真实 XLSX/CSV 的解析结果与统计口径自洽」「删除数据源的三个入口 + 影响面 + 撤销」「02b 与 02c 共用一个表级注册表、状态变更跨页同步」「卡片/列表/抽屉三处同口径给出源端库表总数与已同步数，未纳管的表可一键纳入」「抽屉不再渲染写死的假表」「规则模板库每张卡都带真按钮、点按钮即套用、且套用也进撤销栈」「同步任务向导 5 步（选源表→目标表→字段映射→同步策略→调度配置）能把未纳管的源表一起纳入并只生成一条任务」「任务列表 / 任务详情 / 立即运行的真实状态流转 / 运行监控由运行实例派生 / 失败可达且处理阻塞后重跑即成功」「**码值映射同一字段配了两组对照时预览与 SQL 仍完全一致**：只生效第一组、命中率分母不随重复块虚增、重复块标红并可一键并入」等护栏），每一条都验证「配置 → 预览 → SQL」三者一致
...
- [`docs/PRODUCTION-READINESS-ASSESSMENT.md`](docs/PRODUCTION-READINESS-ASSESSMENT.md) — **商用生产级就绪度评估**：真闭环与假闭环逐项对照（附源码行号）、三个架构级风险、8 类共 40+ 项差距清单（P0/P1/P2）、分三阶段的落地路径
- [`docs/INTEGRATION-MODULE-REVIEW.md`](docs/INTEGRATION-MODULE-REVIEW.md) — **数据集成模块功能闭环与对标核查**：模块现状盘点、8 环链路逐环核对（断点在第 4 环「同步任务配置」）、14 项大厂功能对照、三处内部不一致、按 P0/P1/P2 的补齐建议
