...
> ℹ️ **关于 `?v=` 缓存版本号**：dev server 已发 `no-store`，正常情况下改完刷新即见；但 `pages/*.html` 里**所有**带 `?v=` 的本地脚本，版本号**必须 ≥ 对应文件的 mtime**（`verify-legacy-fixes.js` 逐个校验，当前覆盖 8 处 —— 只盯 `cleaning.js` 是不够的：09-21 就是 `15` 页漏 bump `integration-data.js`，导致「老数据 + 新 UI」）。
...
│   ├── cleaning-task-create.js ← 🧩 「建清洗任务」：让「ODS 有表、清洗侧还没任务」的表能**就地按推荐模板建一条**（字段走 srcFieldNames 的域字段池、角色按名推断、样例行与既有源表同款脏值口径），并落 localStorage 供 02b/02c 复用
...
- 🔎 **切源表是搜索式浮层而非下拉框**：左栏当前表卡片即触发器（点击 / `Enter` / `⇧⌘K`），浮层支持表名·中文名·**数据源名·源端表名**·目标表·字段名全维度检索（命中处高亮）、按**数据源**分成 12 组并可**收起 / 展开**（组标题报「可清洗 / 本组张数」；默认只展开「当前表所在的数据源」，**点数据源名即展开/收起它下面的表**，搜索命中的组会自动展开，右上「展开全部 / 收起全部」一键铺平；**数据源渲染成灰底分区带（粗体 + `ds-00X` 编号 + 「n/m 张」）、表渲染成引导线内的缩进子项（缩进一层、与数据源名左对齐）**，分区带还带 `sticky` 吸附，滚到组内深处也知道自己在哪个数据源）、`↑↓` 移动 / `↵` 切换 / `Esc` 关闭；并在选中行**预知切换后果**——角色齐备度（同类 / 缺哪几个角色）+ 模板适配结果（几项规则会不适用）。后果预知在流水线克隆副本上重放，**绝不改动当前配置**
...
- 🧭 **静默失效零容忍**：726 条运行时 DOM 断言覆盖全部四档改造 + 数据集成模块 + 多轮 UX 简化（含「批处理结果与执行顺序无关」「批处理结束后现场原样还原」「停用正在回放的步骤必给解释性 toast」「toast hover 暂停」「默认日期口径不视为待确认」「❓ helper 默认折叠所有 cfg-note」「左栏单表 UI + 面包屑表名即切源表触发器」「顶栏不再有第二个『切换源表』按钮」「切表后面包屑表名同步刷新」「源表切换浮层的搜索/键盘/点击提交/后果预知纯读不污染配置」「切换器候选换成集成注册表（29 张 / 12 个数据源）后，分组、徽标、计数全部按结构自适应」「标着『未配清洗任务』的表点了/回车都不切表、只给引导 toast」「`?table=` 直达：已配任务就切过去、没配就停在默认表」「跨域扩到 10 张源表后所有断言仍按结构自适应」「拖拽上传真实 XLSX/CSV 的解析结果与统计口径自洽」「删除数据源的三个入口 + 影响面 + 撤销」「02b 与 02c 共用一个表级注册表、状态变更跨页同步」「卡片/列表/抽屉三处同口径给出源端库表总数与已同步数，未纳管的表可一键纳入」「抽屉不再渲染写死的假表」「规则模板库每张卡都带真按钮、点按钮即套用、且套用也进撤销栈」「同步任务向导 5 步（选源表→目标表→字段映射→同步策略→调度配置）能把未纳管的源表一起纳入并只生成一条任务」「任务列表 / 任务详情 / 立即运行的真实状态流转 / 运行监控由运行实例派生 / 失败可达且处理阻塞后重跑即成功」「**码值映射同一字段配了两组对照时预览与 SQL 仍完全一致**：只生效第一组、命中率分母不随重复块虚增，重复块标红并可一键并入」「**源表切换浮层按数据源手风琴**：默认只展开当前表所在那组、点数据源名可展可收、搜索命中组自动展开、`←`/`→` 同效、全部收起后组标题仍在（不会「一收起就点不开」）、**数据源 = 灰底分区带 / 表 = 引导线内的缩进子项**（表名与数据源名左对齐、分区带滚动吸附；不再画会勾在空白里的拐角弧线）」「**所有 `?v=` 都不早于对应文件的 mtime**」等护栏），每一条都验证「配置 → 预览 → SQL」三者一致
- 🧩 **「没有清洗任务」不再是死胡同**：02b / 02c 的入口文案由**唯一判据** `DF.integration.hasCleaningTask(id)` 决定 —— 有任务显示「去清洗」，没任务显示「**＋ 建清洗任务**」（虚线胶囊）。点它会弹确认框（源端位置 / 数据源 / 落库目标 / 字段数与来源 / 识别到的语义角色 / 推荐模板），确认后**按推荐模板生成初始流水线**并直接进入配置：字段取自 `srcFieldNames()` 的域字段池（与「新建同步任务」向导同一份事实）、角色按字段名推断、样例行沿用既有源表同款脏值口径，任务落 localStorage 因此刷新 / 跨页都不丢。数据还没落库的表（未同步 / 同步中）**不给这个入口**，只提示先完成首次同步。
- ⚠ **入口不能比事实更乐观**：判据只允许有一个。`ODS_TABLES[].clean` 与清洗侧 `SOURCES` 由 `verify-srcpick.js` §1b 逐张对钉；任何页面都不得再用 `status === 'synced'` 自己判"能不能去清洗"（这正是 19 个「去清洗」里 9 个是死胡同的成因）。
